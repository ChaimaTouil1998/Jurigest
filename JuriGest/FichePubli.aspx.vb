...
Public Class FichePubli
    ...
    Public Sub Alimenter()
        ...
        commandtext = "Select * from dtlPubli where id = " & txtId.Text
        commande = Connexion.CreateCommand
        commande.CommandText = commandtext
        Connexion.Open()
        lire = commande.ExecuteReader
        While lire.Read()
            dpType.SelectedValue = lire("Journal")
            txtDate.Text = lire("Date")
            If lire("texte") IsNot System.DBNull.Value Then txtTexte.Text = lire("texte")
            If lire("numero") IsNot System.DBNull.Value Then txtNumero.Text = lire("numero")
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        If Trim(txtId.Text) = "" Then
            Dim connectionString = ConfigurationManager.ConnectionStrings("JuriConnectionString").ConnectionString
            Dim commandtext As String
            commandtext = "insert into dtlpubli (NumDOs, journal, date, numero, texte, chemin) values (" & txtCode.Text & "," & dpType.SelectedValue & ", cast('" & txtDate.Text & "' as datetime), N'" & Replace(txtNumero.Text, "'", "''") & "', N'" & Replace(txtTexte.Text, "'", "''") & "', N'" & Replace(txtChemin.Text, "'", "''") & "')"
            Dim Connexion As OdbcConnection = New OdbcConnection(connectionString)
            Dim commande As OdbcCommand = Connexion.CreateCommand
            commande.CommandText = commandtext
            Connexion.Open()
        ...
        Else
            Dim connectionString = ConfigurationManager.ConnectionStrings("JuriConnectionString").ConnectionString
            Dim commandtext As String
            commandtext = "update dtlpubli set chemin = N'" & Replace(txtChemin.Text, "'", "''") & "', texte = '" & Replace(txtTexte.Text, "'", "''") & "', numero = N'" & Replace(txtNumero.Text, "'", "''") & "', journal = " & dpType.SelectedValue & "  Where id = " & txtId.Text
            Dim Connexion As OdbcConnection = New OdbcConnection(connectionString)
            Dim commande As OdbcCommand = Connexion.CreateCommand
            commande.CommandText = commandtext
            Connexion.Open()
    ...
    Private Sub grdDoc_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles grdDoc.RowCommand
        ...
        If (e.CommandName = "SuppLg") Then
            ...
            Dim connectionString = ConfigurationManager.ConnectionStrings("JuriConnectionString").ConnectionString
            Dim commandtext As String
            commandtext = "update dtlpubli set chemin = '' where id = " & txtId.Text
            Dim Connexion As OdbcConnection = New OdbcConnection(connectionString)
            Dim commande As OdbcCommand = Connexion.CreateCommand
            commande.CommandText = commandtext
            Connexion.Open()
