<%@ Page Title="حسابات الإئتمان" Language="vb" AutoEventWireup="false" MasterPageFile="~/Standard.Master" CodeBehind="BanqueDos.aspx.vb" Inherits="JuriGest.BanqueDos" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
     <!-- Page Heading -->
                    <div class="d-sm-flex align-items-center justify-content-between mb-4 text-right">
                        <h1 class="h3 mb-0 text-gray-800">حسابات الإئتمان<asp:ScriptManager ID="ScriptManager1" runat="server">
                            </asp:ScriptManager>
                        </h1>
                    </div>
    ...
    <Columns>
        <asp:BoundField DataField="banque" HeaderText="البنك" SortExpression="banque" />
          <asp:BoundField DataField="DatCpte" HeaderText="تاريخ فتح الحساب" SortExpression="DatCpte" DataFormatString="{0:d}" />
        <asp:BoundField DataField="Compte" HeaderText="الحساب" SortExpression="Compte" />
         <asp:TemplateField>
                 <ItemTemplate>
     ...
     <asp:SqlDataSource ID="sqlDoc" runat="server" ConnectionString="<%$ ConnectionStrings:JuriConnectionString %>" ProviderName="<%$ ConnectionStrings:JuriConnectionString.ProviderName %>" SelectCommand="SELECT [Id], [DatCpte], [Compte], banque.libelle as banque FROM [dtlBanque], banque WHERE dtlbanque.banque = banque.code and  ([NumDos] = ?)">
         <SelectParameters>
