<%@ Page Title="إعلام المتداخلين" Language="vb" AutoEventWireup="false" MasterPageFile="~/Standard.Master" CodeBehind="InfoDos.aspx.vb" Inherits="JuriGest.InfoDos" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
     <!-- Page Heading -->
                    <div class="d-sm-flex align-items-center justify-content-between mb-4 text-right">
                        <h1 class="h3 mb-0 text-gray-800">إعلام المتداخلين<asp:ScriptManager ID="ScriptManager1" runat="server">
                            </asp:ScriptManager>
                        </h1>
                    </div>
    ...
    <Columns>
        <asp:BoundField DataField="nom" HeaderText="الطرف" SortExpression="nom" />
          <asp:BoundField DataField="Date" HeaderText="تاريخ الإعلام" SortExpression="Date" DataFormatString="{0:d}" />
        <asp:TemplateField>
                 <ItemTemplate>
     ...
     <asp:SqlDataSource ID="sqlDoc" runat="server" ConnectionString="<%$ ConnectionStrings:JuriConnectionString %>" ProviderName="<%$ ConnectionStrings:JuriConnectionString.ProviderName %>" SelectCommand="SELECT dtlinfo.[Id], dtlinfo.[Date], dtlpartie.nom FROM [dtlInfo] , dtlpartie WHERE dtlinfo.idpartie = dtlpartie.id and (dtlinfo.[NumDos] = ?)">
         <SelectParameters>
