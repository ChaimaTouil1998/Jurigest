<%@ Page Title="قائمة الأذون" Language="vb" AutoEventWireup="false" MasterPageFile="~/Standard.Master" CodeBehind="OrdreDos.aspx.vb" Inherits="JuriGest.OrdreDos" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
     <!-- Page Heading -->
                    <div class="d-sm-flex align-items-center justify-content-between mb-4 text-right">
                        <h1 class="h3 mb-0 text-gray-800">الأذون<asp:ScriptManager ID="ScriptManager1" runat="server">
                            </asp:ScriptManager>
                        </h1>
                    </div>
    ...
    <Columns>
        <asp:BoundField DataField="NumOrdre" HeaderText="عدد" SortExpression="NumOrdre" />
          <asp:BoundField DataField="Date" HeaderText="التاريخ" SortExpression="Date" DataFormatString="{0:d}" />
        <asp:BoundField DataField="Mt" HeaderText="المبلغ" SortExpression="Mt" DataFormatString="{0:F3}" />
     ...
     <asp:SqlDataSource ID="sqlDoc" runat="server" ConnectionString="<%$ ConnectionStrings:JuriConnectionString %>" ProviderName="<%$ ConnectionStrings:JuriConnectionString.ProviderName %>" SelectCommand="SELECT [NumOrdre], [Id], [Date], [Mt], TypOrdre.libelle as LibType FROM [dtlOrdre], typordre  WHERE dtlordre.type = typordre.code and ([NumDos] = ?)">
         <SelectParameters>
