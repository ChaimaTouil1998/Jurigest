<%@ Page Title="الإشهارات" Language="vb" AutoEventWireup="false" MasterPageFile="~/Standard.Master" CodeBehind="PubliDos.aspx.vb" Inherits="JuriGest.PubliDos" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
     <!-- Page Heading -->
                    <div class="d-sm-flex align-items-center justify-content-between mb-4 text-right">
                        <h1 class="h3 mb-0 text-gray-800">الإشهارات<asp:ScriptManager ID="ScriptManager1" runat="server">
                            </asp:ScriptManager>
                        </h1>
                    </div>
    ...
    <Columns>
        <asp:BoundField DataField="journal" HeaderText="الجريدة" SortExpression="journal" ReadOnly="True" />
          <asp:BoundField DataField="Date" HeaderText="التاريخ" SortExpression="Date" DataFormatString="{0:d}" />
        <asp:BoundField DataField="Numero" HeaderText="العدد" SortExpression="Numero" />
        <asp:TemplateField>
                 <ItemTemplate>
     ...
     <asp:SqlDataSource ID="sqlDoc" runat="server" ConnectionString="<%$ ConnectionStrings:JuriConnectionString %>" ProviderName="<%$ ConnectionStrings:JuriConnectionString.ProviderName %>" SelectCommand="SELECT Id,  [Journal].libelle as journal, [Date], [Numero] FROM [dtlPubli], journal WHERE dtlpubli.journal = journal.code and  ([NumDos] = ?)">
         <SelectParameters>
